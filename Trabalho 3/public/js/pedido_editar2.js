            ...
            updateItemValues: function (produto_index) {
                ...
                if (aux_preco_unitario_desc == 0 || preco_unitario == aux_preco_unitario_desc){
                    ...
                    if (adicional > 0){
                        ...
                        preco_unitario_desc = round(preco_adicional - (preco_adicional * desconto_icms), 2);                            
...
(function ($, carrinhoController) {
   ...
   // var $val_condicao_pagamento = $container3.find('#cond_pagto');    
    ...
    var FreteView = function () {
        ...
        var $cnpj_loading = $render_transportadores_cnpj.parent();
        var check_confirmo = $('#check_confirmo');
        var $tipo_frete_fob = $('#tipo_frete_fob');
        ...
        $.extend(self, {
            buscaItensTransportador: function (transportador_id) {
                ...
                if (transportador_id.length > 0) {
                    ...
                    $.ajax({
                        url: base_url + '/api/transportador/' + transportador[0].id,
                        type: 'GET',
                        error: function () {
                        ...
                        success: function (transportador) {
                            ...
                            $render_transportadores_cnpj.val(transportador.A4_CGC);
            ...
            init: function () {
                ...
                $render_transportadores_obs.html(transp_observacao);                  
            ...
            loading: function (status) {
                var loaderClass = 'loader';
                if(status == true) {
                    $telefone_loading.addClass(loaderClass);
                    $cnpj_loading.addClass(loaderClass);
                } else {
                    $telefone_loading.removeClass(loaderClass);
                    $cnpj_loading.removeClass(loaderClass);
                }
                ...
                itens_do_pedido.forEach(item => {  
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            for(var i = 0; i < produtos_tipos.length; i++) {

                                if(produtos_tipos[i].id == tipo_id) {
                                    var tipo = produtos_tipos[i];
                                }
                            }
                            ...
                            var desconto1 = item.ZIT_DSCVPC / 100;
                            var desconto2 = item.ZIT_DSCMTR / 100;
                            ...
                            var produto = {
                                ...
                                adicional: adicional,
                                desconto_vpc: desconto1,
                                desconto_mostruario: desconto2
                                //st: st_valor
                            };
    ...
    var FiltroView = function () {
        ...
        var input_condpagto = $('#cond_pagto');
        var tabela_condpagto = $('#tabela');
            ...
            init: function () {
                ...
                select_produtos_modelos.selectize({
                    ...
                    onChange: function (modelo_id) {
                        ...
                        var codtab_id = select_condicao_pagamento.val();
                ...
                if (select_clientes.length > 0) {
                    //clienteView.loading(true);
                    let cliente_id = select_clientes.val();
                    ...
                    $.ajax({
                        ...
                        success: function (cliente) {
                            ...
                            select_clientes[0].selectize.lock(); //desabilita o cliente
                        }
                    });
                }
                ...
                select_condicao_venda[0].selectize.lock(); //desabilita a condição de venda
                ...
                select_condicao_pagamento.selectize({
                    ...
                    load: function (query, callback) {
                        ...
                        let cliente = clienteController.get();
                        ...
                        $.ajax({
                            ...
                            success: function (condpag) {
                                ...
                                // Encontrando o desconto relativo a tabela
                                for(let i = 0; i < condpag.length; i++) { 

                                    if(condpag[i].id == condicao_pagamento) {

                                        
                                        input_condpagto.val(condpag[i].id); //seta a cond pag no input hidden
                                        tabela_condpagto.val(condpag[i].cod_tabela);
                                        //seleciona a opção com o item
                                        selectize_condicao_pagamento = select_condicao_pagamento[0].selectize.addItem(condpag[i].id, true);    
                                        //atualiza o value do item
                                        select_condicao_pagamento.val(condpag[i].id);
                                        select_condicao_pagamento[0].selectize.lock(); //desabilita a tabela de venda
                                    }
                                }
