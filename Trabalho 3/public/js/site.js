...
$.fn['appMaskApply'] = function (mask_name) {
    ...
    switch (mask_name) {
        ...
        case 'money2':
            self = self.length != 0 ? self : $(_mask['money2']);
            if (typeof $.fn.maskMoney === 'undefined')
                throw  new Error('Artico requires jQuery-MaskMoney');
            self.appMask('00,000', {reverse: true});
            //self.maskMoney({allowNegative: false, thousands: '.', decimal: ',', affixesStay: false});
            break;
        ...
        case 'cpf':
            self = self.length != 0 ? self : $(_mask['cpf']);
            self.appMask('000.000.000-00');
            break;
        ...
        case 'telefone':
            ...
            var SPMaskBehavior = function (val) {
                    return val.replace(/\D/g, '').length === 11 ? '(000) 0000-0000' : '(000) 00000-00009';
                },
                spOptions = {
        ...
        case 'celular':
            self = self.length != 0 ? self : $(_mask['celular']);
            var SPMaskBehavior = function (val) {
                return val.replace(/\D/g, '').length === 9 ? '00000-0000' : '00000-0000'; 
            },
            spOptions = {
                onKeyPress: function (val, e, field, options){
                    field.mask(SPMaskBehavior.apply({}, arguments), options);
